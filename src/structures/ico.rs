...
use crate::graphics::{helper::calc_normal, pipelines::ico::IcoVertex};
...
impl Ico {
    ...
    pub fn base() -> Self {
        ...
        for face in faces.iter_mut() {
            ...
            face.siblings = [
                edges.get(&[i1, i0]).map(|v| *v),
                edges.get(&[i2, i1]).map(|v| *v),
                edges.get(&[i0, i2]).map(|v| *v),
            ];
        }
        ...
        for f in self.faces.drain(..) {
            ...
            vertices.extend(std::array::IntoIter::new([
                IcoFace {
                    ...
                    indices: [j2, i0, j0],
                    vertices: [n2, f0, n0],
                    normal: calc_normal(n2, f0, n0).into(),
                    siblings: [None; 3],
                },
                IcoFace {
                    ...
                    indices: [j1, i2, j2],
                    vertices: [n1, f2, n2],
                    normal: calc_normal(n1, f2, n2).into(),
                    siblings: [None; 3],
                },
                IcoFace {
                    ...
                    indices: [j0, i1, j1],
                    vertices: [n0, f1, n1],
                    normal: calc_normal(n0, f1, n1).into(),
                    siblings: [None; 3],
                },
                IcoFace {
                    ...
                    indices: [j0, j1, j2],
                    vertices: [n0, n1, n2],
                    normal: calc_normal(n0, n1, n2).into(),
                    siblings: [None; 3],
                },
            ]));
        }
    ...
    pub fn vertex_data(&self) -> Vec<IcoVertex> {
        self.faces
            .iter()
            .flat_map(|f| {
                f.vertices.iter().copied().map(move |v| IcoVertex {
                    position: v.into(),
