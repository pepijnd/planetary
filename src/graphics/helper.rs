...
pub fn create_pipeline(
    ...
    let shaders = shaders();
    let vs_module = shaders.get(vs).unwrap();
    let fs_module = shaders.get(fs).unwrap();
...
pub fn create_texture_binding(device: &wgpu::Device, name: Option<impl Display>) -> TextureLayout {
